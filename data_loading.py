...
class ForestPersonsBaseDataset(Dataset):
    ...
    def __init__(self, root_dir, split='train', modality='rgb', verbose=True):
        ...
        json_paths, explicit_split = self._select_annotation_jsons(root_dir, split)
        if not json_paths:
            if verbose:
                print(f" ForestPersons ({modality}) [{split}]: no COCO annotation json found under {root_dir}")
            return

        image_index = self._build_image_index(root_dir)
        img_key_to_path = {}
        person_category_ids = defaultdict(set)
        raw_annotations = []

        for j_path in json_paths:
            source_key = os.path.normcase(os.path.abspath(j_path))
            try:
                with open(j_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
            except Exception as e:
                if verbose:
                    print(f"  Warning: could not read {j_path}: {e}")
                continue
            ...
            if categories:
                for cat in categories:
                    ...
                    if cat_name in PERSON_CLASS_NAMES or 'person' in cat_name or 'human' in cat_name:
                        person_category_ids[source_key].add(cat.get('id'))
            ...
            for img in data.get('images', []):
                ...
                img_path = self._resolve_image_path(file_name, image_index, split)
                if img_path is None:
                    continue

                img_key = (source_key, img_id)
                img_key_to_path[img_key] = img_path
                self.image_info[img_key] = {
                    'width': img.get('width'),
                    'height': img.get('height'),
                }

            for ann in data.get('annotations', []):
                raw_annotations.append((source_key, ann))

        valid_img_keys = sorted(img_key_to_path.keys(), key=lambda x: (x[0], str(x[1])))
        if not explicit_split:
            import random
            random.Random(RANDOM_SEED).shuffle(valid_img_keys)
            split_idx = int(len(valid_img_keys) * 0.8)
            if split == 'train':
                assigned_img_keys = set(valid_img_keys[:split_idx])
            else:
                assigned_img_keys = set(valid_img_keys[split_idx:])
        else:
            assigned_img_keys = set(valid_img_keys)

        for source_key, ann in raw_annotations:
            img_key = (source_key, ann.get('image_id'))
            if img_key not in assigned_img_keys:
                continue

            allowed_cats = person_category_ids.get(source_key)
            if allowed_cats and ann.get('category_id') not in allowed_cats:
                continue
            ...
            clipped = self._clip_coco_bbox(bbox, self.image_info.get(img_key, {}))
            if clipped is None:
                continue

            xmin, ymin, xmax, ymax = clipped
            self.annotations_map[img_key].append({
                'class_id': map_person_class_id(),
                'xmin': xmin, 'ymin': ymin,
                'xmax': xmax, 'ymax': ymax,
            })

        self.paired_paths = [(img_key_to_path[k], k) for k in valid_img_keys if k in assigned_img_keys]

        if verbose:
            split_note = "official split" if explicit_split else "deterministic 80/20 split"
            print(
                f" ForestPersons ({modality}) [{split}]: "
                f"{len(self.paired_paths)} single-modality samples ({split_note})"
            )
    ...
    @classmethod
    def _select_annotation_jsons(cls, root_dir, split):
        all_jsons = sorted(glob(os.path.join(root_dir, '**', '*.json'), recursive=True))
        coco_jsons = []
        for path in all_jsons:
            name = os.path.basename(path).lower()
            if name.endswith('.json') and any(tok in name for tok in ('train', 'val', 'valid', 'validation', 'test')):
                coco_jsons.append(path)

        if not coco_jsons:
            coco_jsons = all_jsons
        ...
        tokens = split_tokens.get(split, (split,))
        selected = [
            p for p in coco_jsons
            if any(tok in os.path.basename(p).lower() for tok in tokens)
        ]

        if split in ('val', 'valid') and not selected:
            selected = [p for p in coco_jsons if 'test' in os.path.basename(p).lower()]

        if selected:
            return selected, True
        return coco_jsons, False

    @classmethod
    def _build_image_index(cls, root_dir):
        image_paths = [
            p for p in glob(os.path.join(root_dir, '**', '*.*'), recursive=True)
        ...
        for path in image_paths:
            ...
            by_rel[cls._norm_rel(rel)] = path
            by_name[os.path.basename(path).lower()].append(path)
        return {'rel': by_rel, 'name': by_name}

    @classmethod
    def _resolve_image_path(cls, file_name, image_index, split):
        clean_name = str(file_name).replace('\\', '/')
        rel_key = cls._norm_rel(clean_name)
        if rel_key in image_index['rel']:
            return image_index['rel'][rel_key]

        basename = os.path.basename(clean_name).lower()
        candidates = image_index['name'].get(basename, [])
        if not candidates:
            return None
        if len(candidates) == 1:
            return candidates[0]

        split_tokens = [f"/{split.lower()}/"]
        if split in ('val', 'valid'):
            split_tokens.extend(['/val/', '/valid/', '/validation/', '/test/'])
        for candidate in candidates:
            c_norm = candidate.replace('\\', '/').lower()
            if any(tok in c_norm for tok in split_tokens):
                return candidate
        return candidates[0]

    @staticmethod
    def _clip_coco_bbox(bbox, img_info):
        try:
            x, y, w, h = [float(v) for v in bbox[:4]]
        except (TypeError, ValueError):
            return None
        if not all(np.isfinite(v) for v in (x, y, w, h)) or w <= 1 or h <= 1:
            return None
    ...
    def __getitem__(self, idx):
        img_path, img_key = self.paired_paths[idx]
        ...
        if self.modality == 'thermal':
            ...
            if image_thermal is None:
                raise FileNotFoundError(f"Could not read thermal image: {img_path}")
            h_orig, w_orig = image_thermal.shape[:2]
            ...
            if image_bgr is None:
                raise FileNotFoundError(f"Could not read RGB image: {img_path}")
            image_rgb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)
            h_orig, w_orig = image_rgb.shape[:2]
            # Fake thermal (black) for CMM handling
            image_thermal = np.zeros((h_orig, w_orig), dtype=np.uint8)

        annotations = self.annotations_map.get(img_key, [])
        return (image_rgb, image_thermal), annotations, (h_orig, w_orig)

    def iter_annotations(self):
        for _, img_key in self.paired_paths:
            info = self.image_info.get(img_key, {})
            h = info.get('height') or 1
            w = info.get('width') or 1
            yield self.annotations_map.get(img_key, []), (int(h), int(w))
