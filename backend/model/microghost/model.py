...
class EnergyGate(nn.Module):
    ...
    def forward(self, feat_rgb, feat_thm, rgb_present=None, thm_present=None):
        e_rgb = self.proj_rgb(feat_rgb)          # (B, 1, H, W)
        e_thm = self.proj_thm(feat_thm)          # (B, 1, H, W)

        if rgb_present is not None:
            e_rgb = torch.where(rgb_present > 0.5, e_rgb, torch.full_like(e_rgb, -30.0))
        if thm_present is not None:
            e_thm = torch.where(thm_present > 0.5, e_thm, torch.full_like(e_thm, -30.0))
...
class MicroGhostV2(nn.Module):
    ...
    def forward(self, x):
        ...
        fused_s2, w_rgb, w_thm = self.energy_gate(
            feat_rgb_s2, feat_thm_s2, rgb_present=rgb_present, thm_present=thm_present
        )
...
def estimate_peak_sram(model, input_size=None, batch_size=1):
    ...
    with torch.no_grad():
        ...
        x = torch.randn(batch_size, INPUT_CHANNELS, h, w, device=device)
        _ = model(x)
